...
export async function saveAllToSupabase(
  ...
  try {
    ...
    try {
      const dummyStaffIds = ['staff-1', 'staff-2', 'staff-1789528823190-1-64pi7', 'staff-1789529008391-2-hk1tr'];
      const validStaffList = (state.staffList || []).filter((s) => !dummyStaffIds.includes(s.id));
      const staffIds = validStaffList.map((s) => s.id);
      await syncTableRecords('staff_members', staffIds);
      // Clean any accidental dummy records
      await supabase.from('staff_members').delete().in('id', dummyStaffIds);
      if (validStaffList.length > 0) {
        const fullStaffPayload = validStaffList.map((s, idx) => ({
          id: s.id,
          name: s.name,
          role: s.role,
          category: s.category,
          institution: s.institution || 'MI',
          nip_or_nuptk: s.nipOrNuptk || null,
          education: s.education || null,
          subjects: s.subjects || null,
          photo_url: s.photoUrl || null,
          phone: s.phone || null,
          status: s.status || 'Aktif Mengajar',
          bio: s.bio || null,
          quote: s.quote || null,
          service_years: s.serviceYears || null,
          expertise: Array.isArray(s.expertise) ? s.expertise : [],
          order_num: s.order ?? idx,
        }));

        const { error: fullUpsertError } = await supabase.from('staff_members').upsert(fullStaffPayload);
        if (fullUpsertError) {
          console.warn('Upsert staff_members full detail note, falling back to basic columns:', fullUpsertError.message);
...
export async function upsertStaffToSupabase(staff: any) {
  try {
    const fullPayload = {
      ...
      service_years: staff.serviceYears || null,
      expertise: Array.isArray(staff.expertise) ? staff.expertise : [],
      order_num: staff.order ?? 99,
    };

    const { error } = await supabase.from('staff_members').upsert(fullPayload);
    if (error) {
      console.warn('Upsert staff full detail fallback to basic columns:', error.message);
      await supabase.from('staff_members').upsert({
        id: staff.id,
        name: staff.name,
        role: staff.role,
  ...
  try {
    ...
    if (storeRes.status === 'fulfilled' && storeRes.value.data?.data) {
      ...
      if (state && Array.isArray(state.staffList)) {
        state.staffList = state.staffList.filter((s: any) => s && !DUMMY_STAFF_IDS.includes(s.id));
      }
    }
    ...
    if (profileRes.status === 'fulfilled' && profileRes.value.data) {
      ...
      state.schoolProfile = {
        ...
        faviconUrl: (p as any)?.favicon_url || state.schoolProfile?.faviconUrl || p.logo_url || state.schoolProfile?.logoUrl || "/assets/logo-maarif.svg",
    ...
    if (staffRes.status === 'fulfilled' && staffRes.value.data && staffRes.value.data.length > 0) {
      state.staffList = staffRes.value.data
        .filter((s: any) => !DUMMY_STAFF_IDS.includes(s.id))
        .map((s: any, idx: number) => {
          const existingStaffFromStore = state?.staffList?.find((ex: any) => ex.id === s.id);
          return {
            id: s.id,
            name: s.name,
            role: s.role,
            category: s.category as any,
            institution: s.institution || existingStaffFromStore?.institution || (
            ...
            nipOrNuptk: s.nip_or_nuptk || existingStaffFromStore?.nipOrNuptk || '-',
            education: s.education || existingStaffFromStore?.education || '',
            subjects: s.subjects || existingStaffFromStore?.subjects || (s.role?.toLowerCase().includes('kelompok') ? 'Pendidikan Anak Usia Dini' : (s.role === 'Kepala Madrasah' ? 'Manajerial & Kebijakan Madrasah' : s.role)),
            photoUrl: s.photo_url || existingStaffFromStore?.photoUrl || '',
            phone: s.phone || existingStaffFromStore?.phone || '',
            order: s.order_num ?? existingStaffFromStore?.order ?? (idx + 1),
            status: s.status || existingStaffFromStore?.status || 'Aktif',
            bio: (s.bio !== undefined && s.bio !== null ? s.bio : existingStaffFromStore?.bio) || '',
            quote: (s.quote !== undefined && s.quote !== null ? s.quote : existingStaffFromStore?.quote) || '',
            serviceYears: (s.service_years !== undefined && s.service_years !== null ? s.service_years : (s as any).serviceYears) || existingStaffFromStore?.serviceYears || '',
            expertise: Array.isArray(s.expertise)
              ? s.expertise
              : (Array.isArray(existingStaffFromStore?.expertise) ? existingStaffFromStore?.expertise : []),
          };
        });
    }
